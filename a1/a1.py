...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    # We Identify leaf nodes and Initialize there weight
    max_distance = max(node2distances.values())
    nodes_maxdist = [k for k,v in node2distances.items() if v==max_distance]
    # Here we initialize all nodes to one since, leaves start with credit 1 and
    # everytime we move one level up we append one to the node weights
...
def approximate_betweenness(graph, max_depth):
    ...
    for node in nodes:
        ...
        short_frac = bottom_up(node, node2distances, node2num_paths, node2parents)
        for edge in short_frac:
            result[edge] += short_frac[edge]
...
def volume(nodes, graph):
    ...
    count = 0
    for tup in edges_tup:
        if((tup[0] in nodes) or (tup[1] in nodes) ):
            count+=1
...
def cut(S, T, graph):
    ...
    cut_count = 0
    for tup in edges_tup:
        if ((tup[0] in S) and (tup[1] in T)):
            cut_count+= 1
        elif((tup[0] in T) and (tup[1] in S)):
            cut_count+=1
...
def score_max_depths(graph, max_depths):
    ...
    for md in max_depths:
        ...
        result.append((md,norm_cut(component[0],component[1],graph)))
...
def jaccard(graph, node, k):
    ...
    # Node to Itself has highest score 1 so removing it
    sorted_list.pop(0)
...
def path_score(graph, root, k, beta):
    ...
    pass
    graph_nodes = graph.nodes()
    # edges = set(map(frozenset, graph.edges()))
    edges = graph.edges()
