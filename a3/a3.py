...
def featurize(movies):
    ...
    movie_dic = {}
    vocab_set = set()
    df_i_dict = defaultdict(int)
    #N
    num_movie = movies.shape
    num_movies = num_movie[0]
    movies["features"] = ""

    for index,row in movies.iterrows():
        # movie ID --> (num_features,Counter(tokens))
        movie_dic[row["movieId"]] = (len(set(row["tokens"])),Counter(row["tokens"]),index)
        #Building Vocab
    ...
    for movie in sorted(movie_dic):
        # max_k = movie_dic[movie][1].most_common()
        max_k = movie_dic[movie][1].most_common(1)[0][1]
        data = []
        column = []
        # Improve here
        tup = movie_dic[movie]
        for term in tup[1]:
            tf_id = tup[1][term]
            df_i = df_i_dict[term]
            tf_idf = (tf_id / max_k) * math.log((num_movies / df_i), 10)
            data.append(tf_idf)
...
def make_predictions(movies, ratings_train, ratings_test):
    ...
    for index,row in ratings_train.iterrows():
        user_dict[row["userId"]].append((row["movieId"],row["rating"]))
        mean_dict[row["userId"]].append(row["rating"])

    #mean rating for userId
    for user in mean_dict:
        mean_dict[user] = sum(mean_dict[user])/len(mean_dict[user])

    for index,row in ratings_test.iterrows():
        mov_rated_x = user_dict[row["userId"]]
        denom_sum = 0.0
        ...
        for mov in mov_rated_x:
            ...
            vector_b = row_mov_movies
            sim_a_b = cosine_sim(vector_a,vector_b)
            if(sim_a_b >0):
                num_sum += sim_a_b * mov[1]
                denom_sum += sim_a_b
                count_pos +=1
        if(count_pos >0):
            weighted_avg = num_sum/denom_sum
        elif(count_pos==0):
            weighted_avg = mean_dict[row["userId"]]

        # print(weighted_avg)
        ratings_test_copy.set_value(index=index,col="rating",value=weighted_avg)
